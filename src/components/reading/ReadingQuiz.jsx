...
export default function ReadingQuiz({ questions, storyId }) {
  ...
  const handleAnswer = (option) => {
    ...
    if (option === currentQ.answer) {
      ...
      setMascotMessage("Tepat sekali! Membacamu teliti banget! 🎉");
    } else {
      setMascotMessage(`Bukan itu jawabannya. Jawaban yang benar: "${currentQ.answer}". Tetap semangat ya! 💪`);
    }
  };
  ...
  const handleNextQuestion = () => {
    if (currentIndex < questions.length - 1) {
      ...
      setMascotMessage("Lanjut ke pertanyaan pemahaman berikutnya! 🚀");
    } else {
      setIsFinished(true);
  ...
  const resetQuiz = () => {
    ...
    setSelectedAnswer(null);
    setIsFinished(false);
    setMascotMessage("Perhatikan kata-kata penting dalam cerita ya! 🤓");
  };
  ...
  if (isFinished) {
    return (
      ...
      >
        ...
        <h3 className="text-3xl font-black text-indigo-950 mb-2">Kuis Selesai! 🎉</h3>
        <p className="text-purple-900/80 font-bold text-sm mb-6">Hasil kuis pemahaman cerita kamu:</p>
        ...
        <div className="bg-indigo-50/80 p-6 rounded-2xl border border-indigo-100 mb-8">
          ...
          <p className="text-indigo-600 font-extrabold text-sm">Status: Cerita Berhasil Dikuasai!</p>
        </div>
        ...
        >
          <RefreshCcw className="w-5 h-5" /> Ulangi Kuis Cerita
        </button>
      </motion.div>
    );
  }
